    ...
    switch ($action) {
        case "SendMessage": {
                if (isset($_POST["email"]) && !empty($_POST["email"])) {
                    ...
                    $response = (SendEmail($message, $_POST["subject"], $_POST["email"], $email)) ? 'Mensaje enviado' : "Fallo al enviar el mensaje";
                } else {
                    $response = "Fallo al enviar el mensaje";
                }
            }
            break;
        default: {
                $response = "Acción Inválida: " . $action;
            }
    }
}
